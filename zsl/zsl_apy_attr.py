...
import argparse, sys
import time
from datahelpers import datahelpersAPY
...
def get_next_batch(step, n_fullbatch=4, n_trainingExamples=500):
	...
	if step%n_fullbatch == 0 and step!=0:
		return None
		#return range(step*batch_size, n_trainingExamples)
...
def training(train_x, train_y, train_class, test_x, test_y, test_class, attributes):
	...
	with tf.Session() as sess:
		...
		for i in range(n_iteration):
			...
			'''
			indices = get_next_batch(i, n_fullbatch, n_trainingExamples)
			if indices is None:
				continue
			#batch_value = len(indices)
...
def main(train=True, cv=False, test=False):
	...
	features = dataset['vggFeatures']      	# [n_totalExamples, n_input]
	#print[[features[i][j]for j in range(4096)if (features[i][j]<0 or features[i][j]>1)]for i in range(15339)]
	...
	if (cv == True):
		...
		#										test_class, attributes, batch_value) 
		final_train_accuracy, final_test_accuracy = training(train_x, train_y, train_class, test_x, test_y,
